...
if game.PlaceId == LOBBY_ID then
    ...
    FG.Color=ColorSequence.new{ColorSequenceKeypoint.new(0,Color3.fromRGB(35,8,65)),ColorSequenceKeypoint.new(1,Color3.fromRGB(12,2,28))}; FG.Rotation=45
    
    local FT=Instance.new("TextLabel",F)
    FT.Size=UDim2.new(1,0,0,40); FT.Position=UDim2.fromOffset(0,6); FT.BackgroundTransparency=1; FT.Text="QUẢN LÝ HÀNG CHỜ"
    FT.TextColor3=Color3.fromRGB(210,150,255); FT.Font=Enum.Font.GothamBlack; FT.TextSize=18; FT.ZIndex=21

    local function mkSection(title,yOff,items,onSelect,isActive)
        local lbl=Instance.new("TextLabel",F)
        lbl.Size=UDim2.new(1,-32,0,18); lbl.Position=UDim2.fromOffset(16,yOff); lbl.BackgroundTransparency=1; lbl.Text=title
        lbl.TextColor3=Color3.fromRGB(150,100,220); lbl.Font=Enum.Font.GothamBold; lbl.TextSize=12; lbl.TextXAlignment=Enum.TextXAlignment.Left; lbl.ZIndex=21
        local sf=Instance.new("ScrollingFrame",F)
        sf.Size=UDim2.new(1,-32,0,110); sf.Position=UDim2.fromOffset(16,yOff+20); sf.BackgroundColor3=Color3.fromRGB(10,2,22); sf.BackgroundTransparency=0.3
        sf.CanvasSize=UDim2.new(0,0,0,#items*38); sf.ZIndex=21; Instance.new("UICorner",sf).CornerRadius=UDim.new(0,10)
        local ll=Instance.new("UIListLayout",sf); ll.Padding=UDim.new(0,4)
        local pad=Instance.new("UIPadding",sf); pad.PaddingTop=UDim.new(0,6); pad.PaddingLeft=UDim.new(0,6)
        local btns={}
        for _,name in ipairs(items) do
            local btn=Instance.new("TextButton",sf)
            btn.Size=UDim2.new(1,-12,0,30); btn.Font=Enum.Font.GothamBold; btn.TextSize=13; btn.Text=name; btn.ZIndex=22
            ...
            btn.MouseButton1Click:Connect(function()
                for _,b in pairs(btns) do b.BackgroundColor3=Color3.fromRGB(35,15,60); b.TextColor3=Color3.fromRGB(180,140,220) end
                btn.BackgroundColor3=Color3.fromRGB(110,40,200); btn.TextColor3=Color3.new(1,1,1); onSelect(name)
            end)
            btns[name]=btn
        end
    end
    ...
    mkSection("CHỌN ĐỘ KHÓ",195,DiffList, function(v)
        local rv=DiffMap[v]
        if rv=="Best difficulty" then Save.IsBestDiff=true else Save.IsBestDiff=false; Save.SelectedDiff=rv end saveData()
    ...
    local function mkBtn(txt,color,xOff,w,cb)
        ...
        btn.Size=UDim2.new(0,w,0,44); btn.Position=UDim2.fromOffset(xOff,460); btn.BackgroundColor3=color; btn.Text=txt; btn.TextColor3=Color3.new(1,1,1)
        btn.Font=Enum.Font.GothamBlack; btn.TextSize=14; btn.ZIndex=22; Instance.new("UICorner",btn).CornerRadius=UDim.new(0,10)
        btn.MouseButton1Click:Connect(cb); return btn
    end

    local autoBtn=mkBtn("TỰ ĐỘNG: "..(Save.AutoStartQueue and "BẬT" or "TẮT"), Save.AutoStartQueue and Color3.fromRGB(40,140,40) or Color3.fromRGB(140,30,30),16,180, function() end)
    ...
    autoBtn.MouseButton1Click:Connect(function()
        ...
        autoBtn.BackgroundColor3=Save.AutoStartQueue and Color3.fromRGB(40,140,40) or Color3.fromRGB(140,30,30); saveData()
    end)
    ...
    if Save.AutoStartQueue then task.spawn(function() for i=5,1,-1 do startBtn.Text="TỰ ĐỘNG TRONG "..i.."s"; setStatus("Bắt đầu sau "..i.."s..."); task.wait(1) end startBtn.Text="BẮT ĐẦU HÀNG CHỜ"; fireQueue() end) end

-- ============================================================
-- PHẦN TRẬN ĐẤU (TYCOON)
-- ============================================================
elseif game.PlaceId == GAME_ID then
    ...
    local function getUIData()
        local h = lp.PlayerGui.Hud.Upgrade.Holder
        if not h.Visible then return nil end
    ...
    local function updateObjectData(obj)
        ...
        tp(obj)
        for i=1,10 do task.wait(0.2) local d = getUIData()
            if d and d.level > 0 then priceCache[obj] = d return d end
        end return nil
    end

    -- Theo dõi kết thúc trận
    task.spawn(function() while true do if lp.PlayerGui.Menus.GameResult.Visible then
        setStatus("Trận đấu kết thúc - Dịch chuyển..."); pcall(function() Save.TotalGold = Save.TotalGold + cleanNumber(lp.PlayerGui.Hud.Reward.GoldAmount.Text) saveData() end)
    ...
    tp(myBase.Door)
    repeat task.wait(0.5) local d=getUIData() if d then if d.level>=2 then break end if getBalance()>=d.cost then UpgradeEvent:FireServer("upgrade",myBase.Door) end end until false

    -- GIAI ĐOẠN 1b: BÁN FARM KHỞI ĐẦU (Sửa lỗi kẹt yêu cầu Cửa)
    setStatus("Đang bán Farm khởi đầu...")
    local mainFarm = nil
    repeat mainFarm=myBase.Tiles.Starter:FindFirstChild("Farm3") task.wait(0.5) until mainFarm
    tp(mainFarm); task.wait(0.5)
    UpgradeEvent:FireServer("sell", mainFarm)
    task.wait(1)

    -- GIAI ĐOẠN 2: Phân loại ô đất
    ...
    local function getNilByDebugId(name,id) for _,o in pairs(getnilinstances()) do if o.Name==name and o:GetDebugId()==id then return o end end return nil end

    local function sellAllFarms()
        if farmsSold then return end; farmsSold=true
        setStatus("CHẾ ĐỘ THỦ - Đang bán Farm...")
        for _,o in ipairs(buildList) do if o.Name:find("Farm") then 
            local t = getNilByDebugId(o.Name, o:GetDebugId()) or o
            pcall(function() UpgradeEvent:FireServer("sell", t) end) task.wait(0.2) priceCache[o]=nil
        end end
    end

    local function restoreFarms()
        if not farmsSold then return end; farmsSold=false
        setStatus("Đang khôi phục Nông trại...")
    ...
    while true do
        ...
        if best and priceCache[best] and priceCache[best].level >= 4 and dData and dData.level < 6 then best, low = myBase.Door, dData.cost end
        if dData and not dData.isMax and dData.level < (maxLvl + 1) then if dData.cost < low then low, best = dData.cost, myBase.Door end end
        ...
        if best then
            ...
            updateObjectData(myBase.Door); for _,o in ipairs(buildList) do updateObjectData(o) end
        end
        task.wait(1)
    end
end
