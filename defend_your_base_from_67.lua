    ...
    while true do
        ...
        for obj,data in pairs(priceCache) do
            if obj and obj.Parent and not data.isMax then
                ...
                if defenseMode and isFarm then continue end
                if obj~=myBase.Door and not(obj==mainFarm and data.level>=MAIN_FARM_LIMIT) then
                    if not(defenseMode and obj==mainFarm) then
                        if data.cost>0 and data.cost<lowestPrice then lowestPrice,bestTarget=data.cost,obj end
                    end
                end
            end
        end
        ...
        if bestTarget and priceCache[bestTarget] and priceCache[bestTarget].level>=4 and dData and dData.level<6 then
            bestTarget,lowestPrice=myBase.Door,dData.cost
        end
        if dData and not dData.isMax and dData.level<(maxUnitLevel+1) then
            if dData.cost<lowestPrice then lowestPrice,bestTarget=dData.cost,myBase.Door end
        end

        if bestTarget then
            setStatus((defenseMode and "[THỦ] " or "").."-> "..bestTarget.Name
                .." lv"..(priceCache[bestTarget] and priceCache[bestTarget].level or "?")
                .." $"..lowestPrice)
