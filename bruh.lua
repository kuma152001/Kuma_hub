...
task.spawn(function()
    ...
    TabFarm:CreateToggle({
        Name = "⚡ Farm Từ Xa (Instant)",
        CurrentValue = false,
        Callback = function(V) 
            _G.Config.InstantFarm = V
            if V then _G.Config.AutoLoot = false end 
        end
    })
    
    TabFarm:CreateToggle({
        Name = "▶ Farm Thường (Giữ E)",
        CurrentValue = false,
        Callback = function(V) 
            _G.Config.AutoLoot = V
            if V then _G.Config.InstantFarm = false end 
        end
    })
    
    TabFarm:CreateToggle({
        Name = "🌍 Farm Tất Cả (Bỏ lọc)",
        CurrentValue = false,
        Callback = function(V) 
            _G.Config.FarmAll = V 
        end
    ...
    TabTele:CreateButton({ 
        Name = "📍 Lưu vị trí hiện tại (Điểm về)", 
        Callback = function() 
            if LP.Character and LP.Character:FindFirstChild("HumanoidRootPart") then 
    ...
    TabTele:CreateButton({ 
        Name = "➕ Thêm vị trí đứng", 
        Callback = function() 
            if LP.Character and LP.Character:FindFirstChild("HumanoidRootPart") then 
                table.insert(_G.Config.Waypoints, LP.Character.HumanoidRootPart.CFrame)
                WaypointLabel:Set("Điểm đã lưu: " .. #_G.Config.Waypoints) 
            end 
        end
    })
    ...
    TabTele:CreateButton({ 
        ...
        Callback = function() 
            _G.Config.Waypoints = {}
            WaypointLabel:Set("Điểm đã lưu: 0") 
        end
    })
    
    TabTele:CreateToggle({ 
        Name = "▶ Bắt đầu chạy vòng lặp", 
        CurrentValue = false, 
    ...
    local function UpdateKeys() 
        if #_G.Config.ExtraKeys == 0 then 
            SequenceDisplay:Set("Phím hiện tại: [ Trống ]") 
        else 
            SequenceDisplay:Set("Phím: " .. table.concat(_G.Config.ExtraKeys, " -> ")) 
        end 
    end
